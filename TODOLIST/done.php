    ...
    <div id="main">
						...
						<!-- Post -->
							<article class="post">
							 ...
							 <div><h2>List of done</h2> <h1>(View Data)</h1></div>
							...
							-->
								...
								<table class="alt">
											...
											<?php
												...
												$Maker = $_SESSION["C_ID"];
												if($re=="pri"){
													$result=mysql_query("SELECT * FROM `list` WHERE `done` = 'Done' && `EmployeeNo` = '$Maker' ORDER BY priority DESC  ");
												...
												$result=mysql_query("SELECT * FROM list WHERE `done` = 'Done' && `EmployeeNo` = '$Maker'");
												while($test = mysql_fetch_array($result))
													{
													$id = $test['name'];	
													echo "<tr align='center'>";	
						...
						<!-- Post -->
							<div align="center">
								...
								<center>
								<?php
								echo" <li><a href='done.php?query=pri#' class='button big'>Priority</a></li>";
